...
const BASE_SEND_RATE = 1 / 5
const SLOW_SEND_RATE = 1 / 2
...
const info = {
	...
	write() {
		...
		if (!isInTurretSeat) {
			...
			// Access anchor via app.get() using the anchor ID pattern
			if (turretBaseBone) {
				const anchorId = `seat-2-${app.instanceId}`
				const anchor = app.get(anchorId)
				if (anchor && anchor.rotation) {
					anchor.rotation.y = this.turretBaseRotation || 0
				}
			}
		}
	...
	deserialize(data) {
		...
		this.turretBaseRotation = tbr || 0
		this.turretGunRotation = tgr || 0
		this.turretBarrelRotation = tbarrelr || 0
		this.turretHeat = th || 0
		this.isTurretOverheated = toh || false
...
function setupTouchControls() {
	...
	safeExecute(() => {
		...
		// This is just a visual indicator for mobile users
		if (isMobile) {
...
function updateTurretShooting(delta) {
	...
	// Get muzzle position (like pistol script)
	let origin = car.position.clone()
	origin.y += 2 // Fallback height

	if (turretMuzzleBone) {
		try {
			const muzzleMatrixWorld = turretMuzzleBone.matrixWorld
			if (muzzleMatrixWorld) {
				// Get muzzle position from bone
				origin.setFromMatrixPosition(muzzleMatrixWorld)
				// Project forward slightly to avoid self-hits (like pistol script)
				const forwardOffset = dir.clone().multiplyScalar(0.3)
				origin.add(forwardOffset)
			}
		} catch (e) {
			// Bone not found, use fallback position
		}
	}
	...
	// Visual effects
	createTurretMuzzleFlash()
	playTurretSound('Fire')

	// Create bullet trail (client-side visual) - use frozen direction
	const trail = createTurretBulletTrail(origin, frozenDir)
	if (trail) {
		const distance = turretRange
		const speed = 50 // Projectile speed for visual
		let traveled = 0

		// Store frozen direction as array to ensure it's truly immutable in closure
		const frozenDirArray = frozenDir.toArray()

		const updateHandler = (dt) => {
			const step = speed * dt
			traveled += step

			// Use frozen direction from array (truly immutable)
			v2.fromArray(frozenDirArray).multiplyScalar(step)
			trail.position.add(v2)
	...
	function init(_state_) {
		...
		app.on('seat', ([seatIdx, playerId]) => {
			...
			if (seat.playerId === player.id) {
				...
				safeExecute(() => {
					if (wasInTurretSeat) {
						if (turretHUD) {
							car.remove(turretHUD)
							turretHUD = null
							turretHeatBar = null
						}
						removeTurretMobileControls()
					} else if (dashboard) {
						car.remove(dashboard)
						dashboard = null
						speedometer = null
					}
		...
		app.on('update', delta => {
			...
			if (isInTurretSeat && control && turretBaseBone && turretGunBone) {
				...
				// Update player anchor rotation to match turret base (read directly from bone)
				const turretSeat = seats[2]
				if (turretSeat && turretSeat.anchor && turretBaseBone && turretBaseBone.rotation) {
					// Set anchor rotation to match turret base bone rotation (Y-axis only)
					turretSeat.anchor.rotation.y = turretBaseBone.rotation.y
				}

				// Send turret state updates to server (similar to how driver sends vehicle state)
				lastSend += delta
				if (lastSend > adaptiveSendRate) {
					lastSend = 0
					// Read current turret state and send it
					const turretInfo = info.read().serialize()
					app.send('info', turretInfo)
				}
			}
		})
		...
		app.on('turret:fire-visual', (data) => {
			const startPos = new Vector3().fromArray(data.origin)
			const dir = new Vector3().fromArray(data.direction || data.dir).normalize()

			// Create muzzle flash for all clients
			safeExecute(() => {
				const muzzleFlash = app.create('particles', {
					shape: ['sphere', 0.15, 1],
					direction: 1,
			...
			// Create bullet trail for all clients
			const trail = createTurretBulletTrail(startPos, dir)
			if (trail) {
				const distance = turretRange
				const speed = 50 // Projectile speed for visual
				let traveled = 0
...
function viewerMode() {
	...
	app.on('info', data => {
		info.deserialize(data)
		// Write network state to bones (for viewers only - gunner controls their own turret)
		info.write()
	})
	...
	return {
		...
		update(delta) {
			...
			// Enhanced wheel interpolation
			for (let i = 0; i < wheels.length; i++) {
				const wheel = wheels[i]
				if (!wheel.hub || !wheel.tire) continue

				wheel.hub.position.y = lerp(wheel.hub.position.y, info.wheels[i].offset, 6 * delta)
				wheel.hub.rotation.y = lerp(wheel.hub.rotation.y, info.wheels[i].turn, 6 * delta)
				wheel.tire.rotation.x = lerp(wheel.tire.rotation.x, info.wheels[i].rotate, 6 * delta)
				wheel.temperature = lerp(wheel.temperature || 0, info.wheels[i].temperature || 0, 2 * delta)
...
function simulateMode() {
	...
	return {
		...
		update(delta) {
			...
			// Network transmission (include turret state)
			lastSend += delta
			if (lastSend > adaptiveSendRate) {
				lastSend = 0
