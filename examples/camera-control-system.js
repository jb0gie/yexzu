...
if (world.isClient) {
  ...
  app.on('update', () => {
    if (!initialized) {
      ...
      setTimeout(() => {
        ensureCameraHelpersVisible()
        // Sync global helpers toggle to props on init
        world.setShowHelpers?.(props.showHelpers === true)
  ...
  function createCameraPresets(playerPos) {
    cameraPresets.forEach((preset, index) => {
      ...
      const camera = app.create('camera', {
        ...
        // Enable a default feed so users see it work out-of-the-box
        feed: { enabled: true, width: 512, height: 512 },
...
G - Toggle Motion
H - Toggle UI
Right Click - Autofocus`,
      color: '#cccccc',
      fontSize: 14,
      lineHeight: 1.4
    })
    // DOF tuner
    const dofTitle = app.create('uitext', { value: 'DOF (Default Camera)', color: '#9adcff', fontSize: 16, marginBottom: 6 })
    const dofFocus = app.create('uitext', { value: `Focus: ${(world.prefs?.dofFocusDistance ?? 10).toFixed(2)}` , color: '#ffffff', fontSize: 14 })
    const dofRange = app.create('uitext', { value: `Range: ${(world.prefs?.dofFocusRange ?? 5).toFixed(2)}` , color: '#ffffff', fontSize: 14 })
    const dofBokeh = app.create('uitext', { value: `Bokeh: ${(world.prefs?.dofBokehScale ?? 1).toFixed(2)}` , color: '#ffffff', fontSize: 14, marginBottom: 8 })

    const minusFocus = app.create('uitext', { value: '[- Focus]', color: '#ff6b6b', backgroundColor: 'rgba(0,0,0,0.3)', padding: 8, borderRadius: 6, marginRight: 6 })
    const plusFocus = app.create('uitext', { value: '[+ Focus]', color: '#4ecdc4', backgroundColor: 'rgba(0,0,0,0.3)', padding: 8, borderRadius: 6, marginRight: 6 })
    const minusRange = app.create('uitext', { value: '[- Range]', color: '#ff6b6b', backgroundColor: 'rgba(0,0,0,0.3)', padding: 8, borderRadius: 6, marginRight: 6 })
    const plusRange = app.create('uitext', { value: '[+ Range]', color: '#4ecdc4', backgroundColor: 'rgba(0,0,0,0.3)', padding: 8, borderRadius: 6, marginRight: 6 })
    const minusBokeh = app.create('uitext', { value: '[- Bokeh]', color: '#ff6b6b', backgroundColor: 'rgba(0,0,0,0.3)', padding: 8, borderRadius: 6, marginRight: 6 })
    const plusBokeh = app.create('uitext', { value: '[+ Bokeh]', color: '#4ecdc4', backgroundColor: 'rgba(0,0,0,0.3)', padding: 8, borderRadius: 6 })

    function refreshDOFLabels() {
      dofFocus.value = `Focus: ${(world.prefs?.dofFocusDistance ?? 10).toFixed(2)}`
      dofRange.value = `Range: ${(world.prefs?.dofFocusRange ?? 5).toFixed(2)}`
      dofBokeh.value = `Bokeh: ${(world.prefs?.dofBokehScale ?? 1).toFixed(2)}`
    }
    const stepFocus = 1
    const stepRange = 1
    const stepBokeh = 0.1
    minusFocus.onPointerUp = () => {
      const base = world.prefs?.dofFocusDistance ?? 10
      world.dofSetFocusDistance?.(base - stepFocus)
      refreshDOFLabels()
    }
    plusFocus.onPointerUp = () => {
      const base = world.prefs?.dofFocusDistance ?? 10
      world.dofSetFocusDistance?.(base + stepFocus)
      refreshDOFLabels()
    }
    minusRange.onPointerUp = () => {
      const base = world.prefs?.dofFocusRange ?? 5
      world.dofSetFocusRange?.(base - stepRange)
      refreshDOFLabels()
    }
    plusRange.onPointerUp = () => {
      const base = world.prefs?.dofFocusRange ?? 5
      world.dofSetFocusRange?.(base + stepRange)
      refreshDOFLabels()
    }
    minusBokeh.onPointerUp = () => {
      const base = world.prefs?.dofBokehScale ?? 1
      world.dofSetBokehScale?.(Math.max(0, base - stepBokeh))
      refreshDOFLabels()
    }
    plusBokeh.onPointerUp = () => {
      const base = world.prefs?.dofBokehScale ?? 1
      world.dofSetBokehScale?.(base + stepBokeh)
      refreshDOFLabels()
    }

    settingsUI.add(dofTitle)
    settingsUI.add(dofFocus)
    settingsUI.add(dofRange)
    settingsUI.add(dofBokeh)
    settingsUI.add(minusFocus)
    settingsUI.add(plusFocus)
    settingsUI.add(minusRange)
    settingsUI.add(plusRange)
    settingsUI.add(minusBokeh)
    settingsUI.add(plusBokeh)
    ...
    // Add a sample “monitor” using Image node with camera feed
    const monitor = app.create('image', {
      src: 'feed://active',
      height: 0.8,
      fit: 'cover',
      color: 'black',
  ...
  function toggleDOF() {
    ...
    camera.setDOF?.({ enabled: !camera.dof?.enabled })
    
    const on = camera.dof?.enabled
    console.log(`[Camera] DOF ${on ? 'ENABLED' : 'DISABLED'} on ${cameraPresets[currentCameraIndex].name}`)
  }
  ...
  function toggleMotion() {
    ...
    const camera = cameras[currentCameraIndex]
    camera.setMotionEnabled?.(!camera.motion?.enabled)
    
    const on = camera.motion?.enabled
    console.log(`[Camera] Motion ${on ? 'ENABLED' : 'DISABLED'} on ${cameraPresets[currentCameraIndex].name}`)
  }
  ...
  function updateFromConfig() {
    ...
    // Update global helpers preference too
    world.setShowHelpers?.(props.showHelpers === true)
    ...
    cameras.forEach((camera, index) => {
      if (camera) {
        ...
        // Update motion
        camera.setMotionEnabled?.(!!props.enableMotion)
        
        // Update DOF
        camera.setDOF?.({ enabled: !!props.enableDOF })
      }
    })
